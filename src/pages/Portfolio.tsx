...
export default function PortfolioPage() {
  ...
  const [transferModal, setTransferModal] = useState<{ workId: string; share: number } | null>(
    null,
  );
              ...
              if (workVal) {
                ...
                owned.push({
                  ...
                  createdAt: work.created_at
                    ? new Date(Number(work.created_at) * 1000).toLocaleDateString()
                    : '',
                });
              }
            }
    ...
    <>
      <main className="mx-auto max-w-4xl px-4 py-12">
        ...
        <p className="text-sm mb-10 text-[rgb(var(--ink-muted))]">
          View all registered medical data structures where your institution holds clinical access
          rights. Transfer data rights to co-investigators.
        </p>
        ...
        {!isConnected ? (
          <div className="rounded-none border border-[rgb(var(--hairline))] bg-[rgb(var(--surface))] p-10 text-center">
            ...
            <p className="text-sm text-[rgb(var(--ink-muted))]">
              Connect your institutional wallet to view your portfolio.
            </p>
          </div>
        ) : loading ? (
          <div className="rounded-none border border-[rgb(var(--hairline))] bg-[rgb(var(--surface))] p-10 text-center">
          ...
          <div className="rounded-none border border-[rgb(var(--hairline))] bg-[rgb(var(--surface))] p-10 text-center">
            ...
            />
            <p className="font-semibold mb-1 text-[rgb(var(--ink-muted))]">
              No clinical records found.
            </p>
            <p className="text-xs" style={{ color: 'rgb(var(--ink-faint))' }}>
              Register a medical record or receive access rights to see them here.
            </p>
          </div>
        ) : (
        ...
        {transferModal && (
          <div
            ...
            style={{ background: 'rgb(0 0 0 / 0.7)', backdropFilter: 'blur(12px)' }}
          >
            <div className="rounded-none border border-[rgb(var(--hairline))] bg-[rgb(var(--surface))] w-full max-w-md p-6 space-y-4">
              ...
              <p className="text-xs text-[rgb(var(--ink-muted))]">
                Record #{transferModal.workId} · Your current rights:{' '}
                {(transferModal.share / 100).toFixed(2)}%
              </p>
