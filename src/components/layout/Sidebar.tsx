...
export default function Sidebar({ isOpen, onClose }: { isOpen?: boolean; onClose?: () => void }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 bottom-0 z-40 w-[260px] flex-col border-r border-[rgb(var(--hairline))] bg-[rgb(var(--surface))] transition-transform duration-300 md:flex md:translate-x-0 ${isOpen ? 'translate-x-0 flex' : '-translate-x-full hidden'}`}
      >
        {/* Brand */}
                ...
                <div className="flex flex-col">
                  {group.links.map(({ href, label, icon: Icon }) => {
                    ...
                    return (
                      ...
                      >
                        <Icon
                          className={`w-5 h-5 ${active ? 'text-[rgb(var(--brand))]' : 'opacity-60'}`}
                        />
                        {label}
                      </Link>
                    );
